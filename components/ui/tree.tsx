...
const itemStyles = tv({
    ...
    variants: {
        isExpanded: {
            true: '[&_[slot=chevron]_svg]:text-foreground [&_[slot=chevron]_.indicator]:rotate-0 [&_[slot=chevron]_svg]:transition [&_[slot=chevron]_svg]:duration-300'
        },
        isFocusVisible: {
...
const Indicator = () => {
    return (
        <Aria.Button className='shrink-0 relative' slot='chevron'>
            <div
                className='ml-auto relative indicator flex items-center justify-center size-5'
            >
                <IconMinus className='absolute duration-300 size-3' />
                <IconMinus
                    className='absolute indicator duration-300 size-3'
                />
            </div>
        </Aria.Button>
    )
